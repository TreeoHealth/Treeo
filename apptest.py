...
# THIS IS TO FIX "Unread result found error"
tmpcursor = cnx.cursor(buffered=True)
...
def check_login():
    if(request.form['username'] in mySQL_adminDB.getAllAdminUsers(cursor, cnx)):
        result = mySQL_adminDB.verifyAdminLogin(
            request.form['username'], request.form['password'], cursor, cnx)
        if(result == False):
            return render_template('login.html', errorMsg="Incorrect username or password.")
        else:
            acct_info = mySQL_userDB.getAcctFromUsername(
                request.form['username'], cursor, cnx)
    ...
    result = mySQL_userDB.checkUserLogin(
        request.form['username'], request.form['password'], cursor, cnx)
...
def displayLoggedInHome():
    if(session.get('logged_in_d')):
        ...
        return render_template('homePageDr.html',
                               docStat=docStatus,
                               docType=mySQL_userDB.getDrTypeOfAcct(
                                   session['username'], cursor, cnx),
                               name=session['name'])
...
def new_register():
    ...
    if reply == "success":
        ...
        if(docStatus == 'provider'):
            ...
            sendAutomatedAcctMsg(
                request.form['username'], "Welcome to Treeo!", emailBody)
            return render_template('login.html', errorMsg="You have not been verified. Check your email for updates.")
...
def approveForm(username):
    ...
    emailBody = "Hello "+session['name']+",\r\nYou have approved "+providerAcctName + \
        " (" + username+") as a care provider. If this was a mistake, please remedy immediately.\r\nSincerely,\r\n    Your Treeo Team"
...
def assignTeam():  # submit update form
    ...
    if(mySQL_userDB.isDrDietitian(provider1, cursor, cnx) == False):
        ...
        mySQL_userDB.assignPatientCareTeam(
            request.form['username'], provider1, provider2, provider3, cursor, cnx)
        emailBody = "Hello,\r\nYou have been assigned a care team.\r\n\r\n\tDietitian: "
        emailBody = emailBody+mySQL_userDB.getNameFromUsername(
            provider1, cursor, cnx)+" ("+provider1+")\r\n\tPhysician: "
        emailBody = emailBody+mySQL_userDB.getNameFromUsername(
            provider2, cursor, cnx)+" ("+provider2+")\r\n\tHealth Coach: "
        emailBody = emailBody + \
            mySQL_userDB.getNameFromUsername(
                provider3, cursor, cnx)+" ("+provider3+")\r\n"
        emailBody = emailBody + \
            "\r\nPlease reach out with any questions or concerns.\r\nSincerely,\r\n    Your Treeo Team"
        sendAutomatedAcctMsg(
            request.form['username'], "Care Team Assignment", emailBody)

        emailBody = "Hello,\r\nYou have been assigned to a patient: "
        emailBody = emailBody+mySQL_userDB.getNameFromUsername(
            request.form['username'], cursor, cnx)+" ("+request.form['username'] + ")\r\n Here is your team:\r\n\r\n\tDietitian: "
        emailBody = emailBody+mySQL_userDB.getNameFromUsername(
            provider1, cursor, cnx)+" ("+provider1+")\r\n\tPhysician: "
        emailBody = emailBody+mySQL_userDB.getNameFromUsername(
            provider2, cursor, cnx)+" ("+provider2+")\r\n\tHealth Coach: "
...
def unapproveForm(username):
    ...
    providerAcctName = mySQL_userDB.getNameFromUsername(username, cursor, cnx)
    emailBody = "Hello "+providerAcctName + \
        ",\r\You have been suspended from being a care provider temporarily.\r\n\r\nLet us know if you have any questions.\r\nSincerely,\r\n    Your Treeo Team"
    sendAutomatedAcctMsg(username, "Provider Account Suspended", emailBody)
    emailBody = "Hello "+session['name']+",\r\nYou have removed provider approval for "+providerAcctName + \
        " (" + username+"). If this was a mistake, please remedy immediately.\r\nSincerely,\r\n    Your Treeo Team"
    sendAutomatedAcctMsg(session['username'],
                         "Provider Approval Revoked", emailBody)

    providerAccType = mySQL_userDB.getDrTypeOfAcct(username, cursor, cnx)
    assignedPatients = mySQL_userDB.returnPatientsAssignedToDr(
        username, cursor, cnx)
    for patientUser in assignedPatients:
        emailBody = "Hello "+patientUser+",\r\nYour "+providerAccType + \
            " ("+providerAcctName+" - "+username + \
            ") on your care team has been removed from the system.\r\nWe will assign a replacement ASAP.\r\nSincerely,\r\n\tYour Treeo Team"
...
def providerAcctPage(username):
    ...
    providerInfoObj = mySQL_userDB.getAcctFromUsername(
        str(username), cursor, cnx)
...
def list_assigned_patients():
    listStr = mySQL_userDB.returnPatientsAssignedToDr(
        session['username'], cursor, cnx)
    listStr.sort()
    patientPages = []
    currPg = 0
...
def nextPg():
    ...
    if(len(patientPages) == 1):
        ...
        return render_template('middlePgSize.html',
                               ...
                               default=selectSize,
                               startResultNum=((currPg)*numPatientsOnPg)+1,
                               endResultNum=((currPg)*numPatientsOnPg) +
                               1+numPatientsOnPg,  # +5
                               totalResultNum=totalNumPatients,
                               currPgNum=currPg+1,
                               options=patientPages[currPg],
...
def patientAcct(username):
    ...
    patientInfoObj = mySQL_userDB.getAcctFromUsername(
        str(username), cursor, cnx)
...
def search_page():
    ...
    actualUsername = (query.split(" - "))[0]
    acct_search_item = mySQL_userDB.getAcctFromUsername(
        actualUsername, cursor, cnx)
    ...
    if(len(query.split(" - ")) == 2 and mySQL_userDB.isUsernameTaken(actualUsername, cursor, cnx)):
        ...
        patientInfoObj = mySQL_userDB.getAcctFromUsername(
            str(actualUsername), cursor, cnx)
        providerList = mySQL_userDB.getCareTeamOfUser(
            str(actualUsername), cursor, cnx)
        return render_template('patientAcctDetails.html',
...
def acct_details():
    acct_info = mySQL_userDB.getAcctFromUsername(
        str(session['username']), cursor, cnx)
    return render_template('ownAcctPg.html',
                           username=acct_info.username,
                           docstatus=("patient" if type(
                               acct_info) == patientUserClass else acct_info.providerType),
...
def deleteAccount():
    ...
    userAcctObj = mySQL_userDB.getAcctFromUsername(
        providerUsername, cursor, cnx)

    if(type(userAcctObj) == providerUserClass):
        assignedPatients = mySQL_userDB.returnPatientsAssignedToDr(
            providerUsername, cursor, cnx)
        for patientUser in assignedPatients:
            emailBody = "Hello "+patientUser+",\r\nYour "+userAcctObj.providerType + \
                " ("+userAcctObj.fname+" "+userAcctObj.lname+" - "+providerUsername + \
                ") on your care team has been removed from the system.\r\nWe will assign a replacement ASAP.\r\nSincerely,\r\n\tYour Treeo Team"
            sendAutomatedAcctMsg(patientUser, "Care Team Revision", emailBody)

        mySQL_userDB.removeDrFromAllCareTeams(providerUsername, cursor, cnx)

    allAppts = mySQL_apptDB.getAllApptsFromUsername(
        providerUsername, tmpcursor, cursor, cnx)
...
def editAcctDetails():
    ...
    acct_info = mySQL_userDB.userAcctInfo(
        str(request.form['username']), cursor, cnx)
    pwUpdate = False
    errMsg = ""
    errFlag = False
    ...
    if(pwUpdate == False):
        response = mySQL_userDB.updateUserAcct(session['username'], str(
            request.form['email']), request.form['fname'], request.form['lname'], "", cursor, cnx)
    else:
        response = mySQL_userDB.updateUserAcct(session['username'], str(
            request.form['email']), request.form['fname'], request.form['lname'], newPw1, cursor, cnx)
    emailBody = "Hello " + \
...
def autoDeleteMtg(mtgid):
    ...
    if (mySQL_apptDB.isMeetingIDValid(str(mtgid), cursor, cnx) == True):
        mtgDetailsObject = mySQL_apptDB.getApptFromMtgId(
            str(mtgid), cursor, cnx)

        emailBody = "Hello "+mtgDetailsObject.patient+",\r\nYour appointment with "+mySQL_userDB.getNameFromUsername(
            mtgDetailsObject.provider, cursor, cnx)+" ("+mtgDetailsObject.provider+") has been cancelled. \r\n\r\n\t"
        ...
        sendAutomatedApptMsg(mtgDetailsObject.patient,
                             "Appointment Cancelled", emailBody)
        emailBody = "Hello "+mtgDetailsObject.provider+",\r\nYour appointment with "+mySQL_userDB.getNameFromUsername(
            mtgDetailsObject.patient, cursor, cnx)+" ("+mtgDetailsObject.patient+") has been cancelled. \r\n\r\n\t"
        emailBody = emailBody+"Appointment details: \r\nDate: "+mtgDetailsObject.startTime[:10]+"\r\nTime: "+mtgDetailsObject.startTime[11:] + \
            "\r\n\r\nThis has been removed from your calendar. Let us know if there are any issues or you wish to reschedule.\r\nSincerely,\r\n\tYour Treeo Team"
        sendAutomatedApptMsg(mtgDetailsObject.provider,
                             "Appointment Cancelled", emailBody)
...
def create_mtg():
    ...
    try:
        if len(request.form['patientUser'].split(" - ")) > 1:
            ...
            jsonResp = zoomtest_post.createMtg(time, str(
                request.form['password']), session['username'], username, cursor, cnx)
        else:
            jsonResp = zoomtest_post.createMtg(time, str(
                request.form['password']), session['username'], request.form['patientUser'], cursor, cnx)
        date = time[:10]
        finalStr = ""
        ...
        apptClassObj = mySQL_apptDB.getApptFromMtgId(
            jsonResp.get("id"), cursor, cnx)
        patientUsername = request.form['patientUser'].split(" - ")[0]
        emailBody = "Hello "+patientUsername+",\r\n\r\nAn appointment has been created for you by " + \
            mySQL_userDB.getNameFromUsername(
                session['username'], cursor, cnx)+" ("+session['username']+"). \r\n\r\n\t"
        emailBody = emailBody+"Appointment details: \r\nDate: "+date+"\r\nTime: " + \
            time[11:]+"\r\nJoin URL: "+apptClassObj.joinURL + \
            "\r\n\r\nLet us know if there are any issures or you wish to cancel.\r\nSincerely,\r\n\tYour Treeo Team"
        sendAutomatedApptMsg(
            patientUsername, "New Appointment Scheduled", emailBody)
        emailBody = "Hello "+session['username']+",\r\nYou have created an appointment for " + \
            mySQL_userDB.getNameFromUsername(
                patientUsername, cursor, cnx)+" ("+patientUsername+"). \r\n\r\n\t"
        emailBody = emailBody+"Appointment details: \r\nDate: "+date+"\r\n\r\nTime: " + \
            time[11:]+"\r\nJoin URL: "+apptClassObj.joinURL + \
            "\r\n\r\nLet us know if there are any issures or you wish to cancel.\r\nSincerely,\r\n\tYour Treeo Team"
        sendAutomatedApptMsg(session['username'],
                             "New Appointment Scheduled", emailBody)
...
def createUserSearch():
    ...
    for username in listPatients:
        if(query.lower() in username.lower()):
            jsonSuggest.append(
                {'value': username, 'data': username.split(" - ")[0]})
...
def editPgFromID():
    ...
    time = mySQL_apptDB.getApptFromMtgId(
        request.form['mtgnum'], cursor, cnx).startTime
    #split and display
    date = time[:10]
    if(time[-1] == 'Z'):
...
def editSubmit():
    ...
    jsonResp = zoomtest_post.updateMtg(str(request.form['mtgnum']), str(
        request.form['mtgname']), time, cursor, cnx)

    jsonResp = zoomtest_post.getMtgFromMtgID(str(request.form['mtgnum']))

    mtgDetails = mySQL_apptDB.getApptFromMtgId(
        str(request.form['mtgnum']), cursor, cnx)
    ...
    emailBody = "Hello "+mtgDetails.patient+",\r\nYour appointment with "+mySQL_userDB.getNameFromUsername(
        mtgDetails.patient, cursor, cnx)+" ("+mtgDetails.provider+") has been updated. \r\n\r\n\t"
    ...
    emailBody = "Hello "+mtgDetails.provider+",\r\nYour appointment with "+mySQL_userDB.getNameFromUsername(
        mtgDetails.patient, cursor, cnx)+" ("+mtgDetails.patient+") has been updated. \r\n\r\n\t"
    emailBody = emailBody+"Updated appointment details: \r\nDate: "+date+"\r\nTime: " + \
        time[11:]+"\r\nJoinURL: "+mtgDetails.joinURL + \
    ...
    # if it is passed so it should not be edited
    if(mySQL_apptDB.isMtgStartTimePassed(request.form['mtgnum'], cursor, cnx) == True):
        return render_template('apptDetail.html',
...
@app.route('/data')
def return_data():
    arrOfMtgs = mySQL_apptDB.getAllApptsFromUsername(
        session['username'], tmpcursor, cursor, cnx)
    # [{ "title": "Meeting",
    ...
    for apptClassItem in arrOfMtgs:
        # skip invalid items that will crash the calendar
        if(apptClassItem.meetingID == "None" or len(apptClassItem.meetingID) != 11):
            continue
        else:
            #[mI, mN, sT]
        ...
        jsonMtgObj = {"title": apptClassItem.meetingName, "start": time,
                      "end": strend, "url": ("/showmtgdetail/"+mtgid)}
...
def show_care_team():
    userObj = mySQL_userDB.getAcctFromUsername(
        session['username'], cursor, cnx)
    providerList = mySQL_userDB.getCareTeamOfUser(
        session['username'], cursor, cnx)
...
def show_mtgdetail(mtgid):
    ...
    if(session.get('logged_in_p')):
        ...
        # if it is passed so it should not be edited
        if(mySQL_apptDB.isMtgStartTimePassed(mtgid, cursor, cnx) == True):
            return render_template('apptDetail.html',
                                   mtgnum=mtgid,
                                   provider=docUser,
                                   patient=patUser,
...
def formatEmail():
    ...
    if(len(query.split(" - ")) == 2):  # if they chose from dropdown
        ...
        return render_template("newEmail.html",
                               inboxUnread=countUnreadInInbox(
                                   session['username']),
                               trashUnread=countUnreadInTrash(
                                   session['username']),
                               sender_username=session['username'],
...
def getAllMessages(username):
    ...
    for (send_date, send_time, subject, read_status, messageID, sender) in cursor:
        if read_status == 'unread':
            msgList.append([str(send_date + " - " + send_time),
                            messageID, sender, send_date, subject, True])
        else:
            msgList.append([str(send_date + " - " + send_time),
                            messageID, sender, send_date, subject, False])

    msgList.sort(reverse=True, key=lambda date: datetime.strptime(
        date[0], "%B %d, %Y - %H:%M:%S"))
    return msgList
...
def getAllMessagesPaged(username, pgNum):  # page to be rendered
    ...
    # this is the final page (not not first)
    if((pageNumber*pageSize+(pageSize) >= len(msgList)) and pageNumber != 0):
    ...
    # this is the first page and there is a next page
    elif(pageNumber == 0 and pageSize < len(msgList)):
        return render_template("messageInbox.html",
                               inboxUnread=countUnreadInInbox(
                                   session['username']),
                               trashUnread=countUnreadInTrash(
...
def getAllMessagesSent(username):
    ...
    for (send_date, send_time, subject, messageID, reciever) in cursor:
        msgList.append([str(send_date + " - " + send_time),
                        messageID, "To:", reciever, send_date, subject])
...
def getAllMsgsInConvo(convoID):
    ...
    for (send_date, send_time, sender, subject, messageID, msgbody, reciever) in cursor:
        ...
        send = str(sender + " - " +
                   mySQL_userDB.getNameFromUsername(sender, tmpcursor, cnx))
        recieve = str(
            reciever + " - " + mySQL_userDB.getNameFromUsername(reciever, tmpcursor, cnx))
        convoList.append([dateWhole, messageID, send,
                          recieve, subject, msgbody])

    convoList.sort(reverse=True, key=lambda date: datetime.strptime(
        date[0], "%B %d, %Y   %H:%M:%S"))
    return convoList
    ...
    for (send_date, send_time, read_status, reciever_loc, subject, perm_del, messageID, sender, sender_loc, reciever) in cursor:
        ...
        if (reciever == username and reciever_loc == 'trash' and perm_del != 'r') or (sender == username and sender_loc == 'trash' and perm_del != 's'):
            if(reciever == username and read_status == 'unread'):
                trashList.append(
                    [dateWhole, messageID, "", sender, send_date, subject, True])
            elif(sender == username):
                trashList.append([dateWhole, messageID, "To:",
                                  reciever, send_date, subject, False])
            else:
                trashList.append(
                    [dateWhole, messageID, "", sender, send_date, subject, False])
    trashList.sort(reverse=True, key=lambda date: datetime.strptime(
        date[0], "%B %d, %Y - %H:%M:%S"))
    return trashList
...
def insertMessage(sender, reciever, subject, body, convoID):
    ...
    insertContent = (msgID, sender, reciever, subject,
                     ...
                     str(datetime.now().strftime('%H:%M:%S')), str(
    ...
    try:
        for msgID in msgIDList:
            query = ("SELECT sender FROM messageDB "
                     "WHERE messageID = %s")
            # NOTE: even if there is only 1 condition, you have to make the item passed to the query into a TUPLE
...
def reply():
    ...
    return render_template("replyEmail.html",
                           ...
                           sender_username=session['username'],
                           reciever_username=(
                               originalReciever if originalSender == session['username'] else originalSender),
                           subject=subj,
                           email_body="")
...
def usernameSearch(box):
    ...
    if(session['logged_in_d'] == True):
        ...
        listPatients = mySQL_userDB.getCareTeamOfUserEmailList(
            session['username'], cursor, cnx)
    for username in listPatients:
